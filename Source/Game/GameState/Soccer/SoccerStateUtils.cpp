...
void CSoccerGameState::CollisionDetection()
{
	for (int i = 0; i < numOfBalls; i++)
	{
			...
			{
				if(balls[i].m_PlayerID == Player2->m_MyBodyIndex)
				{
					Player1->AddHeat(Player1->m_HeatFromBlocking);
				}
				else
				{
			...
			{
				if(balls[i].m_PlayerID == Player1->m_MyBodyIndex)
				{
					Player2->AddHeat(Player2->m_HeatFromBlocking);
				}
				else
				{
	...
	{
		if(m_Scene.HaveTheTwoBodiesCollided(balls[0].m_MyBodyIndex,Targets[i].m_MyBodyIndex))
		{
			//if even
			if( (i % 2) )
			{
	...
	//Pickup only your cannon bitches.....
	if(m_Scene.HaveTheTwoBodiesCollided(Player2->m_MyBodyIndex, XCannon->m_MyBodyIndex))
	{
		Player2->AttachCannon(XCannon);
	}

	if(m_Scene.HaveTheTwoBodiesCollided(Player1->m_MyBodyIndex, YCannon->m_MyBodyIndex))
	{
		Player1->AttachCannon(YCannon);
	}
...
void CSoccerGameState::Controls()
{
...
void CSoccerGameState::GameRun()
{
	...
	if (pInput->Controller1JoyPress(START) || pInput->Controller2JoyPress(START) || pInput->KeyboardKeyPress(DIK_ESCAPE))
	{
	...
	//Camera Update.
	Camera0->LookAt(&balls[0]);
	Camera1->FollowLookAt(&balls[0]);
...
void CSoccerGameState::Initalize2D()
{
...
void CSoccerGameState::Pause()
{
	...
	if( (pInput->Controller1JoyY() < -200) || (pInput->Controller2JoyY() < -200) || ( pInput->KeyboardKeyPress(DIK_UP) )  )
	{
		Me->m_MyBody.Add2Force( CVector3f(0.0f,-1.0f,0.0f) * (MeForce * 4.0f) );
	}
	if( (pInput->Controller1JoyY() > 200) || (pInput->Controller2JoyY() > 200) || ( pInput->KeyboardKeyPress(DIK_DOWN) ) )
	{	
		Me->m_MyBody.Add2Force( CVector3f(0.0f,1.0f,0.0f) * MeForce );
	}
	if( (pInput->Controller1JoyX() < -200) || (pInput->Controller2JoyX() < -200) || ( pInput->KeyboardKeyPress(DIK_LEFT) )  )
	{
		Me->m_MyBody.Add2Force( CVector3f(-1.0f,0.0f,0.0f) * MeForce );
	}
	if( (pInput->Controller1JoyX() > 200) || (pInput->Controller1JoyX() > 200) || ( pInput->KeyboardKeyPress(DIK_RIGHT) ) )
	{
		Me->m_MyBody.Add2Force( CVector3f(1.0f,0.0f,0.0f) * MeForce );
	} 
...
void CSoccerGameState::InitalizeObjects()
{
	...
	ShieldPlayer1->m_MyBody.radius = 20.0f;
